...
def save_results_to_csv(
    rows, schema, job_name: str, project_name: str = None, output_dir: str = "output"
) -> str:
...
def generate_svg_chart(df, chart_type: str, project_name: str, job_name: str) -> str:
    ...
    for i in range(5):
        ...
        y_pos = scale_y(y_val)
        svg_parts.append(
            f'    <line x1="{margin}" y1="{y_pos}" x2="{width - margin}" y2="{y_pos}" stroke="{colors["border"]}" stroke-width="1" opacity="0.5"/>'
        )
    ...
    for i in range(6):
        ...
        x_pos = scale_x(x_val)
        svg_parts.append(
            f'    <line x1="{x_pos}" y1="{margin}" x2="{x_pos}" y2="{height - margin}" stroke="{colors["border"]}" stroke-width="1" opacity="0.5"/>'
        )
    ...
    if chart_type == "simple":
        ...
        svg_parts.append(
            f'    <path d="{area_path}" fill="url(#areaGradient)" opacity="0.6"/>'
        )
        ...
        svg_parts.append(
            f'    <path d="{line_path}" stroke="{colors["primary"]}" stroke-width="3" fill="none" filter="url(#shadow)"/>'
        )

        # Data points
        for i, (x, y) in enumerate(zip(date_nums, downloads)):
            svg_parts.append(
                f'    <circle cx="{scale_x(x)}" cy="{scale_y(y)}" r="4" fill="{colors["primary"]}" stroke="{colors["white"]}" stroke-width="2"/>'
            )
        ...
        for v_idx, version in enumerate(sorted(versions)):
            ...
            if len(version_dates) > 1:
                line_path = (
                    f"M {scale_x(version_dates[0])} {scale_y(version_downloads[0])}"
                )
                for i in range(1, len(version_dates)):
                    line_path += f" L {scale_x(version_dates[i])} {scale_y(version_downloads[i])}"

                svg_parts.append(
                    f'    <path d="{line_path}" stroke="{color}" stroke-width="2" fill="none" filter="url(#shadow)"/>'
                )

            # Data points for this version
            for x, y in zip(version_dates, version_downloads):
                svg_parts.append(
                    f'    <circle cx="{scale_x(x)}" cy="{scale_y(y)}" r="3" fill="{color}" stroke="{colors["white"]}" stroke-width="1.5"/>'
                )
    ...
    for i in range(5):
        ...
        svg_parts.append(
            f'    <text x="{margin - 10}" y="{y_pos + 4}" text-anchor="end" font-family="system-ui, sans-serif" font-size="12" fill="{colors["text_light"]}">{label}</text>'
        )
    ...
    for i in range(6):
        ...
        if i < len(dates):
            ...
            svg_parts.append(
                f'    <text x="{x_pos}" y="{height - margin + 20}" text-anchor="middle" font-family="system-ui, sans-serif" font-size="12" fill="{colors["text_light"]}">{label}</text>'
            )
    ...
    svg_parts.append(
        f'    <text x="{width // 2}" y="30" text-anchor="middle" font-family="system-ui, sans-serif" font-size="18" font-weight="600" fill="{colors["text"]}">{title}</text>'
    )

    # Axis titles
    svg_parts.append(
        f'    <text x="{width // 2}" y="{height - 15}" text-anchor="middle" font-family="system-ui, sans-serif" font-size="14" fill="{colors["text_light"]}">Date</text>'
    )
    svg_parts.append(
        f'    <text x="15" y="{height // 2}" text-anchor="middle" font-family="system-ui, sans-serif" font-size="14" fill="{colors["text_light"]}" transform="rotate(-90, 15, {height // 2})">Daily Downloads</text>'
    )
    ...
    if chart_type == "version":
        ...
        for v_idx, version in enumerate(sorted(versions)):
            ...
            svg_parts.append(
                f'    <circle cx="{legend_x}" cy="{legend_y_pos}" r="6" fill="{color}"/>'
            )
            svg_parts.append(
                f'    <text x="{legend_x + 15}" y="{legend_y_pos + 4}" font-family="system-ui, sans-serif" font-size="12" fill="{colors["text"]}">v{version}</text>'
            )
...
def generate_pie_chart_svg(df, project_name: str) -> str:
    ...
    # Title
    svg_parts.append(
        f'    <text x="{width // 2}" y="30" text-anchor="middle" font-family="system-ui, sans-serif" font-size="18" font-weight="bold" fill="#2c3e50">Recent 30 Days Installer Statistics - {project_name}</text>'
    )
    ...
    for i, (_, row) in enumerate(df.iterrows()):
        ...
        svg_parts.append(
            f'    <path d="{arc_path}" fill="{color}" filter="url(#shadow)"/>'
        )
        ...
        if segment_angle > 15:  # Only label segments larger than 15 degrees
            ...
            # Add percentage text
            svg_parts.append(
                f'    <text x="{label_x}" y="{label_y}" text-anchor="middle" font-family="system-ui, sans-serif" font-size="12" font-weight="bold" fill="white">{percentage}%</text>'
            )
    ...
    svg_parts.append(
        f'    <text x="{legend_x}" y="{legend_y - 10}" font-family="system-ui, sans-serif" font-size="14" font-weight="bold" fill="#2c3e50">Installers:</text>'
    )
    ...
    for i, (_, row) in enumerate(df.iterrows()):
        ...
        # Legend item
        svg_parts.append(
            f'    <rect x="{legend_x}" y="{y_pos - 8}" width="12" height="12" fill="{color}"/>'
        )
        svg_parts.append(
            f'    <text x="{legend_x + 20}" y="{y_pos}" font-family="system-ui, sans-serif" font-size="11" fill="#2c3e50">{installer_name} ({percentage}% - {download_count:,})</text>'
        )

    # Add total downloads info
    total_text = f"Total Downloads: {total_downloads:,}"
    svg_parts.append(
        f'    <text x="{width // 2}" y="{height - 20}" text-anchor="middle" font-family="system-ui, sans-serif" font-size="14" font-weight="bold" fill="#2c3e50">{total_text}</text>'
    )
...
def create_svg_chart(
    rows, schema, job_name: str, project_name: str, output_dir: str = "output"
) -> str:
...
def create_installer_pie_chart(
    rows, schema, project_name: str, output_dir: str = "output"
) -> str:
...
def generate_country_pie_chart_svg(df, project_name: str) -> str:
    ...
    # Title
    svg_parts.append(
        f'    <text x="{width // 2}" y="30" text-anchor="middle" font-family="system-ui, sans-serif" font-size="18" font-weight="bold" fill="#2c3e50">Recent 30 Days Download by Country - {project_name}</text>'
    )
    ...
    svg_parts.append(
        f'    <text x="{legend_x}" y="{legend_y - 10}" font-family="system-ui, sans-serif" font-size="14" font-weight="bold" fill="#2c3e50">Countries:</text>'
    )
    ...
    for i, (_, row) in enumerate(df.iterrows()):
        ...
        svg_parts.append(
            f'    <text x="{legend_x + 20}" y="{y_pos}" font-family="system-ui, sans-serif" font-size="11" fill="#2c3e50">{country_code} ({percentage}% - {download_count:,})</text>'
        )
...
def save_total_downloads_badge(
    rows, schema, project_name: str, output_dir: str = "output"
) -> str:
    ...
    project_output_dir = (
        os.path.join(output_dir, project_name) if project_name else output_dir
    )
...
def save_recent_30_days_badge(
    ...
    recent_downloads_file = os.path.join(
        project_output_dir, "recent_30_days_downloads.txt"
    )
...
def generate_project_html(
    project_name: str,
    output_dir: str = "output",
    template_path: str = "templates/index.html",
) -> str:
...
def generate_chart_sections(svg_files: list, project_name: str) -> str:
    ...
    for svg_file in sorted_files:
        title = chart_descriptions.get(
            svg_file, svg_file.replace(".svg", "").replace("-", " ").title()
        )
        sections += f'''
    <div class="chart-section">
...
def execute_bigquery_job(job_name: str, job_config: dict):
    ...
    try:
        ...
        if job_type == "total_downloads":
            ...
            formatted_recent = format_number(recent_downloads)
            print(
                f"Recent 30 Days Downloads: {recent_downloads:,} ({formatted_recent})"
            )
        elif job_type == "installer_stats_30d":
            ...
            for row in rows:
                ...
                print(
                    f"{installer_name:<20} {download_count:<12,} {percentage:<10.2f}%"
                )
        elif job_type == "download_by_country_30d":
            ...
            for i, row in enumerate(rows):
                if i >= 20:  # Show first 20 rows
                    break
                print(
                    f"{row.download_date.strftime('%Y-%m-%d'):<12} {row.version:<15} {row.daily_downloads:<10}"
                )
            ...
            if len(rows) > 1:
                ...
                print("-" * 30)
                for version, total in sorted(
                    version_totals.items(), key=lambda x: x[1], reverse=True
                ):
            ...
            for i, row in enumerate(rows):
                if i >= 20:  # Show first 20 rows
                    break
                print(
                    f"{row.download_date.strftime('%Y-%m-%d'):<12} {row.daily_downloads:<10}"
                )
